...
def feature_to_spline(feature: Feature) -> Optional[dict]:
    ...
    # throughout, same as the terrain stamps (see userLayers.py's
...
def _circle_spline(cx: float, cz: float, radius: float) -> dict:
    ...
    userLayers.py's build_registration_mark_stamps for the matching
...
def build_registration_mark_splines(course_size_m: float) -> list[dict]:
    ...
    the spline counterpart to userLayers.py's
    ...
    """
    from course_output.userLayers import registration_mark_corners, REGISTRATION_MARK_CIRCLE_RADIUS_M
    return [
